...
Внутри второго архива заодно лежат `openpyxl`, `pillow` и `et_xmlfile` —
те самые, без которых не работает сама утилита. Если на закрытом сервере
их нет, ставьте оттуда же.
